    ...
    const result = await requestDeduplicator.deduplicate(
      ...
      async () => {
        ...
        if (apiSelection.strategy === 'both') {
          ...
          amadeusResult = { status: 'fulfilled', value: await amadeusAPI.searchFlights({ ...body, origin: originCity, destination: destCity, departureDate: depDate, returnDate: retDate }) };
          duffelResult = { status: 'fulfilled', value: { data: [] } };
        } else {
          amadeusResult = { status: 'fulfilled', value: { data: [], dictionaries: {} } };
          duffelResult = { status: 'fulfilled', value: await duffelAPI.searchFlights({ origin: originCity, destination: destCity, departureDate: depDate, returnDate: retDate, adults: body.adults }) };
        }
