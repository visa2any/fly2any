...
export function HotelCard({
  ...
  return (
    ...
    >
      ...
      {/* TABLET/DESKTOP: Enhanced 3-column horizontal layout */}
      <div className="hidden sm:flex flex-row" onClick={() => onViewDetails(hotel.id)}>
        ...
        <div className="relative w-[200px] min-w-[200px] lg:w-[280px] lg:min-w-[280px] h-[180px] lg:h-[200px] flex-shrink-0 overflow-hidden bg-slate-100" onMouseEnter={fetchImages}>
          ...
          {images.length > 1 && (
            <>
              <button onClick={prevImage} className="absolute left-2 top-1/2 -translate-y-1/2 w-7 h-7 flex items-center justify-center rounded-full bg-black/40 backdrop-blur-sm text-white opacity-0 group-hover:opacity-100 transition-opacity">
                <ChevronLeft className="w-4 h-4" />
              </button>
              <button onClick={nextImage} className="absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 flex items-center justify-center rounded-full bg-black/40 backdrop-blur-sm text-white opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          {/* Rating badge */}
          {hotel.reviewScore > 0 && (
            <div className={`absolute bottom-2 right-2 ${reviewCategory.bg} text-white px-2 py-1 rounded-lg text-sm font-bold shadow-lg`}>
        ...
        {/* Column 2: Hotel Info (expanded) */}
        <div className="flex-1 p-3 lg:p-4 flex flex-col min-w-0 border-r border-gray-100">
          {/* Hotel name + stars */}
          <div className="mb-2">
            <h3 className="font-semibold text-[#1d1d1f] text-sm lg:text-[15px] leading-tight line-clamp-2 mb-1.5 tracking-tight">{hotel.name}</h3>
            <div className="flex items-center gap-3 flex-wrap">
              {hotel.rating > 0 && (
                <div className="flex items-center gap-1">
                  <div className="flex">{Array.from({ length: Math.min(hotel.rating, 5) }, (_, i) => <Star key={i} className="w-3.5 h-3.5 fill-amber-400 text-amber-400" />)}</div>
          ...
          <div className="flex items-center gap-2 flex-wrap mt-auto">
            {hasFreeCancellation && (
              <span className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-50 border border-emerald-200 rounded-lg text-[11px] font-semibold text-emerald-700">
                <Shield className="w-3.5 h-3.5" />Free Cancellation
              </span>
            )}
            {hasBreakfast && (
        ...
        {/* Column 3: Price + CTA (right side) */}
        <div className="w-[180px] lg:w-[200px] flex-shrink-0 p-3 lg:p-4 flex flex-col justify-between bg-gradient-to-br from-[#FAFBFC] to-[#F5F5F7]">
          {perNightPrice > 0 ? (
            <>
              {/* Price section */}
              <div>
                ...
                <div className="text-[11px] text-[#86868b] mt-1 space-y-0.5">
                  <div className="line-through text-[#aaa]">{currencySymbol}{Math.round(perNightPrice * 1.15)}</div>
                  <div className="flex items-center gap-1 text-emerald-600 font-medium">
                    <Receipt className="w-3 h-3" />
                    {currencySymbol}{Math.round(totalPrice).toLocaleString()} total
                  </div>
                  <div className="text-[10px]">{nights} nights · {t.inclTax}</div>
                </div>
              </div>

              {/* Guest info */}
              <div className="flex items-center gap-1 text-[10px] text-[#86868b] my-2 py-2 border-t border-b border-gray-100">
                <Users className="w-3 h-3" />
                <span>{adults + children} {t.guests}</span>
                {rooms > 1 && <span>· {rooms} rooms</span>}
              </div>

              {/* CTA Button */}
              <button onClick={(e) => { e.stopPropagation(); handleBooking(); }}
                className="w-full px-4 py-3 font-semibold text-sm rounded-xl text-white active:scale-95 transition-all hover:shadow-lg"
                style={{
                  background: 'linear-gradient(135deg, #EF4136 0%, #DC3A30 100%)',
                  boxShadow: '0 4px 14px -3px rgba(239,65,54,0.4)'
                }}>
                {t.bookNow}
              </button>
            </>
            ...
            <div className="flex flex-col justify-center items-center h-full">
              <span className="text-xs text-[#86868b] mb-3">Check availability</span>
              <button onClick={(e) => { e.stopPropagation(); handleBooking(); }}
                className="w-full px-4 py-2.5 font-semibold text-sm rounded-xl text-white"
                style={{ background: 'linear-gradient(135deg, #EF4136 0%, #DC3A30 100%)' }}>
                {t.view}
              </button>
            </div>
          )}
