...
export function MobileHomeSearchWrapper({
  ...
  const transitionGuardRef = useRef(false); // Prevents rapid-fire state changes
  ...
  const setViewStateGuarded = useCallback((newState: ViewState) => {
    if (transitionGuardRef.current) return;
    transitionGuardRef.current = true;
    setViewState(newState);
    // Release guard after transition completes (matches CSS transition duration)
  ...
  const handleExpand = useCallback(() => {
    triggerHaptic();
    setViewStateGuarded('expanded');
  }, [triggerHaptic, setViewStateGuarded]);
  ...
  const handleSearchSubmit = useCallback(() => {
    ...
    onSearch?.();
  }, [triggerHaptic, setViewStateGuarded, onSearch]);

  // CSS transition classes for smooth state changes (replaces Framer Motion AnimatePresence)
  ...
  if (!hasMounted) {
    ...
    // CRITICAL: Match exact dimensions of EnhancedSearchBar for zero layout shift
    return (
      <div className={`w-full ${glassmorphism ? 'mx-4 md:mx-6' : ''}`} style={{ minHeight: '140px' }}>
        <div className={`${glassmorphism ? 'bg-black/20 backdrop-blur-xl border border-white/10 rounded-2xl' : 'bg-white shadow-lg lg:shadow-xl border-b border-neutral-100'}`} style={{ paddingTop: '16px', paddingBottom: '16px' }}>
          {/* MaxWidthContainer equivalent */}
          <div className="max-w-7xl mx-auto px-0 lg:px-8">
            <div className="animate-pulse">
              {/* Service Tabs skeleton - matches real tabs: min-h-[44px], px-4 py-2.5, gap-1.5/2 */}
              {!hideTabs && (
                <div 
  ...
  return (
    <div ref={wrapperRef} className={`mobile-search-wrapper md:hidden ${glassmorphism ? 'px-4' : ''}`}>
      ...
      <div
        className={`${transitionClasses} ${viewState === 'collapsed' ? 'opacity-100' : 'opacity-0 max-h-0 overflow-hidden pointer-events-none absolute'}`}
        aria-hidden={viewState !== 'collapsed'}
      >
      ...
      <div
        className={`${transitionClasses} ${viewState === 'expanded' ? 'opacity-100' : 'opacity-0 max-h-0 overflow-hidden pointer-events-none absolute'}`}
        aria-hidden={viewState !== 'expanded'}
      >
