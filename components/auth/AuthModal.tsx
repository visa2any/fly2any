...
// Check if Google OAuth is configured (set in Vercel env vars)
const isGoogleAuthConfigured = !!process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID;
...
export function AuthModal({ isOpen, onClose, onSuccess, flightContext }: AuthModalProps) {
  ...
  // Handle Google OAuth - redirect-based (more reliable than popup)
  const handleGoogleSignIn = async () => {
    ...
    try {
      // Use NextAuth signIn with redirect - more reliable than popup
      await signIn('google', {
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white rounded-xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto animate-scaleIn">
        ...
        <div className="p-5">
          ...
          {/* Social Login - Only show if configured */}
          ...
          {/* Divider - only show when Google auth is available */}
          {isGoogleAuthConfigured && (
