...
/**
 * Adventure Activities API
 * Zip lines, ATVs, hiking, climbing, outdoor adventures
 */

const ADVENTURE_KEYWORDS = [
  'adventure', 'zip', 'zipline', 'atv', 'quad', 'rafting', 'bungee',
  'paragliding', 'skydiving', 'hiking', 'climbing', 'jungle', 'safari',
  'canyoning', 'rappelling', 'off-road', 'buggy', '4x4'
];

const EXCLUDE_KEYWORDS = ['transfer', 'shuttle', 'airport'];

function isAdventureExperience(activity: any): boolean {
  const text = `${activity.name || ''} ${activity.shortDescription || ''}`.toLowerCase();
  const hasKeyword = ADVENTURE_KEYWORDS.some(kw => text.includes(kw));
...
function applyMarkup(activity: any): any {
  ...
  return {
    ...activity,
    category: 'adventure',
    categoryName: 'Adventure Activities',
    categoryIcon: '🏔️',
...
export async function GET(request: NextRequest) {
  return handleApiError(request, async () => {
    ...
    const cacheKey = generateCacheKey('experiences:adventure:v1', { lat: Math.round(latitude * 100) / 100, lng: Math.round(longitude * 100) / 100, r: radius });
    ...
    try {
      ...
      if (result?.data) experiences = result.data.filter(isAdventureExperience).map(applyMarkup);
    } catch (error: any) {
      console.error('Adventure search failed:', error.message);
    }

    const response = {
      success: true,
      category: { id: 'adventure', name: 'Adventure Activities', icon: '🏔️' },
      data: experiences,
