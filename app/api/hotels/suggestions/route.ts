 ...
 * Auto-generates city data when not found in database
 */
async function geocodeCityFallback(query: string): Promise<CityDestination[]> {
  try {
    ...
    const res = await fetch(url, {
      headers: { 'User-Agent': 'Fly2Any/1.0 (travel platform)' },
    ...
    return data
      .filter((item: any) => item.lat && item.lon && item.address)
      .map((item: any): CityDestination => {
        const addr = item.address || {};
        const city = addr.city || addr.town || addr.village || addr.municipality || item.name || query;
        ...
        return {
          ...
          type: 'city',
          emoji: '📍',
          flag: countryCode ? String.fromCodePoint(...countryCode.split('').map(c => 127397 + c.charCodeAt(0))) : '🌍',
          categories: ['City'],
          popularity: 5,
          description: `📍 New destination - we're adding this to our database!`,
        };
      });
  } catch (error) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Auto-generate via Nominatim if no results found
    if (normalizedMerged.length === 0 && query.length >= 3) {
      console.log(`🔍 No results for "${query}" - trying Nominatim fallback...`);

      const geocodedResults = await geocodeCityFallback(query);

      if (geocodedResults.length > 0) {
        console.log(`✅ Nominatim found ${geocodedResults.length} results for "${query}"`);

        const geoNormalized = geocodedResults.map(normalizeForFrontend);

        // Track for future database addition
        fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'https://fly2any.com'}/api/analytics/missing-city`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            query,
            source: 'hotel-suggestions',
            autoGenerated: true,
            geocoded: geocodedResults[0]
          }),
        }).catch(() => {});
        ...
        return NextResponse.json({
          ...
          data: geoNormalized,
          meta: {
            count: geoNormalized.length,
            query,
            source: 'nominatim-fallback',
            message: 'New destination found! We are adding it to our database.',
          },
        }, {
      ...
      // Track truly missing cities
      fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'https://fly2any.com'}/api/analytics/missing-city`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ query, source: 'hotel-suggestions', notFound: true }),
      }).catch(() => {});
    }
