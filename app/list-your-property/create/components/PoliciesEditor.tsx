'use client';

import { useState } from 'react';
import { Clock, Shield, Plus, X, Cigarette, Dog, Music, Users, Leaf } from 'lucide-react';
import { CancellationPolicyType, PetPolicy, SmokingPolicy } from '@/lib/properties/types';

interface PoliciesEditorProps {
  checkInTime: string;
  checkOutTime: string;
  cancellationPolicy: CancellationPolicyType;
  petPolicy: PetPolicy;
  smokingPolicy: SmokingPolicy;
  houseRules: string[];
  ecoFeatures: string[];
  onChange: (updates: any) => void;
}

const COMMON_RULES = [
  'No parties or events',
  'Quiet hours after 10:00 PM',
  'No shoes inside',
  'Turn off lights when leaving',
  'No unregistered guests',
];

const ECO_FEATURES = [
  'Solar panels',
  'Recycling bins available',
  'Composting',
  'LED lighting',
  'Rainwater harvesting',
  'Electric vehicle charger',
];

export function PoliciesEditor({
  checkInTime,
  checkOutTime,
  cancellationPolicy,
  petPolicy,
  smokingPolicy,
  houseRules,
  ecoFeatures,
  onChange,
}: PoliciesEditorProps) {
  const [newRule, setNewRule] = useState('');

  const addRule = () => {
    if (newRule && !houseRules.includes(newRule)) {
      onChange({ houseRules: [...houseRules, newRule] });
      setNewRule('');
    }
  };

  const toggleRule = (rule: string) => {
    if (houseRules.includes(rule)) {
      onChange({ houseRules: houseRules.filter(r => r !== rule) });
    } else {
      onChange({ houseRules: [...houseRules, rule] });
    }
  };

  const toggleEcoFeature = (feature: string) => {
    if (ecoFeatures.includes(feature)) {
      onChange({ ecoFeatures: ecoFeatures.filter(f => f !== feature) });
    } else {
      onChange({ ecoFeatures: [...ecoFeatures, feature] });
    }
  };

  return (
    <div className="space-y-8">
      {/* Time Settings */}
      <section className="space-y-4">
        <h3 className="text-white font-bold flex items-center gap-2">
          <Clock className="w-5 h-5 text-amber-400" /> Check-in & Check-out
        </h3>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-bold text-white/50 uppercase mb-1 block">Check-in After</label>
            <input 
              type="time" 
              value={checkInTime}
              onChange={(e) => onChange({ checkInTime: e.target.value })}
              className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white focus:border-amber-400/50 outline-none"
            />
          </div>
          <div>
            <label className="text-xs font-bold text-white/50 uppercase mb-1 block">Check-out Before</label>
            <input 
              type="time" 
              value={checkOutTime}
              onChange={(e) => onChange({ checkOutTime: e.target.value })}
              className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white focus:border-amber-400/50 outline-none"
            />
          </div>
        </div>
      </section>

      {/* Policies */}
      <section className="space-y-4">
        <h3 className="text-white font-bold flex items-center gap-2">
          <Shield className="w-5 h-5 text-amber-400" /> Core Policies
        </h3>
        
        <div className="grid md:grid-cols-2 gap-4">
          <div className="p-4 rounded-xl bg-white/5 border border-white/10">
            <div className="flex items-center gap-2 mb-3 text-white font-semibold">
              <Dog className="w-4 h-4" /> Pet Policy
            </div>
            <select 
              value={petPolicy || 'not_allowed'}
              onChange={(e) => onChange({ petPolicy: e.target.value })}
              className="w-full px-3 py-2 rounded-lg bg-black/20 border border-white/10 text-white outline-none [&>option]:text-black"
            >
              <option value="not_allowed">No pets allowed</option>
              <option value="allowed">Pets allowed</option>
              <option value="allowed_with_fee">Allowed (Extra fee)</option>
            </select>
          </div>

          <div className="p-4 rounded-xl bg-white/5 border border-white/10">
            <div className="flex items-center gap-2 mb-3 text-white font-semibold">
              <Cigarette className="w-4 h-4" /> Smoking Policy
            </div>
            <select 
              value={smokingPolicy || 'not_allowed'}
              onChange={(e) => onChange({ smokingPolicy: e.target.value })}
              className="w-full px-3 py-2 rounded-lg bg-black/20 border border-white/10 text-white outline-none [&>option]:text-black"
            >
              <option value="not_allowed">No, entirely smoke-free</option>
              <option value="designated_areas">Designated areas only</option>
              <option value="allowed">Allowed</option>
            </select>
          </div>
           
           <div className="p-4 rounded-xl bg-white/5 border border-white/10 md:col-span-2">
            <div className="flex items-center gap-2 mb-3 text-white font-semibold">
              <Shield className="w-4 h-4" /> Cancellation Policy
            </div>
            <select 
              value={cancellationPolicy}
              onChange={(e) => onChange({ cancellationPolicy: e.target.value })}
              className="w-full px-3 py-2 rounded-lg bg-black/20 border border-white/10 text-white outline-none [&>option]:text-black"
            >
              <option value="flexible">Flexible (Full refund 1 day prior)</option>
              <option value="moderate">Moderate (Full refund 5 days prior)</option>
              <option value="strict">Strict (Full refund 30 days prior)</option>
              <option value="super_strict">Super Strict (No refunds)</option>
            </select>
          </div>
        </div>
      </section>

      {/* House Rules */}
      <section className="space-y-4">
        <h3 className="text-white font-bold flex items-center gap-2">
          <Music className="w-5 h-5 text-amber-400" /> House Rules
        </h3>
        
        <div className="flex flex-wrap gap-2">
          {COMMON_RULES.map(rule => (
            <button
              key={rule}
              onClick={() => toggleRule(rule)}
              className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-colors ${
                houseRules.includes(rule) 
                  ? 'bg-blue-500/20 border-blue-500/50 text-blue-300' 
                  : 'bg-white/5 border-white/10 text-white/50 hover:bg-white/10'
              }`}
            >
              {rule}
            </button>
          ))}
        </div>

        <div className="flex gap-2">
          <input 
            type="text" 
            value={newRule}
            onChange={(e) => setNewRule(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && addRule()}
            placeholder="Add a custom rule..."
            className="flex-1 px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-white focus:border-amber-400/50 outline-none"
          />
          <button 
            onClick={addRule}
            className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-white font-bold"
          >
            Add
          </button>
        </div>

        <ul className="space-y-2">
          {houseRules.map((rule, idx) => (
             <li key={idx} className="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/5">
                <span className="text-white/80 text-sm">• {rule}</span>
                <button onClick={() => onChange({ houseRules: houseRules.filter(r => r !== rule) })} className="text-white/30 hover:text-red-400">
                   <X className="w-4 h-4" />
                </button>
             </li>
          ))}
        </ul>
      </section>

      {/* Eco Features */}
      <section className="space-y-4">
        <h3 className="text-white font-bold flex items-center gap-2">
          <Leaf className="w-5 h-5 text-emerald-400" /> Sustainability
        </h3>
        <p className="text-white/50 text-sm">Tell guests about your eco-friendly features.</p>

        <div className="grid grid-cols-2 gap-3">
          {ECO_FEATURES.map(feature => (
             <button
               key={feature}
               onClick={() => toggleEcoFeature(feature)}
               className={`p-3 rounded-xl border text-left transition-all flex items-center justify-between ${
                 ecoFeatures.includes(feature)
                   ? 'bg-emerald-500/10 border-emerald-500/50 text-emerald-300'
                   : 'bg-white/5 border-white/10 text-white/60 hover:text-white'
               }`}
             >
               <span className="text-sm font-medium">{feature}</span>
               {ecoFeatures.includes(feature) && <Leaf className="w-3 h-3" />}
             </button>
          ))}
        </div>
      </section>
    </div>
  );
}
