  ...
  export async function PropertiesList({ userId }: { userId: string }) {
      try {
          ...
          if (!properties) throw new Error("Failed to fetch properties");
          ...
          console.error("PropertiesList Error:", e);
          return (
              <div className="p-8 rounded-2xl bg-red-50 border border-red-100 text-center">
                  <h3 className="text-lg font-bold text-red-900 mb-2">Unable to load properties</h3>
                  <p className="text-red-700 mb-4">You might not be connected to the database.</p>
                  <button className="px-4 py-2 bg-white text-red-600 font-bold rounded-lg border border-red-200 hover:bg-red-50">
                      Try Again
                  </button>
              </div>
          );
