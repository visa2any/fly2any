...
export default function DatePickerFixed({
  value,
  onChange,
  placeholder,
  label,
  ...
  useEffect(() => {
    ...
    console.log('DatePicker mounted:', {
      ...
      createPortalAvailable: !!createPortal
    });
  }, []);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (containerRef.current && !containerRef.current.contains(event.target as Node)) {
        console.log('DatePicker: Click outside detected, closing calendar');
        setIsOpen(false);
      }
    };
  ...
  const handleDateSelect = (day: CalendarDay) => {
    ...
    console.log('DatePicker: Date selected:', day.date);
  ...
  const updateCalendarPosition = () => {
    if (containerRef.current) {
      try {
        ...
        console.log('DatePicker: Position updated:', newPosition);
        setCalendarPosition(newPosition);
      } catch (error) {
        console.error('DatePicker: Error updating position:', error);
      }
    } else {
      console.warn('DatePicker: containerRef.current is null');
    }
  };

  const handleToggleOpen = () => {
    console.log('DatePicker: Toggle open clicked, current state:', isOpen);
    ...
    if (!isOpen) {
      ...
      setTimeout(() => {
        setIsOpen(true);
        console.log('DatePicker: Calendar opened');
      }, 10);
    } else {
      setIsOpen(false);
      console.log('DatePicker: Calendar closed');
    }
  };
  ...
  useEffect(() => {
    if (isOpen) {
      console.log('DatePicker: Calendar should be visible', {
        isClient,
  ...
  }, [isOpen, isClient, portalTarget, calendarPosition, calendarDays.length]);
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      {/* Calendar Dropdown with improved portal implementation */}
      {isOpen && isClient && portalTarget && createPortal(
        <div
          ...
          style={{
            ...
            maxHeight: '400px',
            overflow: 'auto',
            visibility: calendarPosition.top > 0 ? 'visible' : 'hidden'
          }}
        ...
        >
          ...
          {process.env.NODE_ENV === 'development' && (
            <div className="mt-2 text-xs text-gray-400 border-t pt-2">
              Debug: isOpen={isOpen.toString()}, pos=({calendarPosition.left},{calendarPosition.top})
            </div>
          )}
        </div>,
        portalTarget
