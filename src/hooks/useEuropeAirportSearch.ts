...
export interface UseEuropeAirportSearchOptions extends EuropeAirportSearchOptions {
  debounceMs?: number;
  cacheResults?: boolean;
  fallbackToApi?: boolean;
}

export interface UseEuropeAirportSearchReturn {
  results: EuropeAirportSearchResult[];
  loading: boolean;
  error: string | null;
  search: (query: string) => Promise<void>;
  clearResults: () => void;
  getAirportByCode: (code: string) => EuropeAirport | null;
  popularAirports: EuropeAirport[];
  totalResults: number;
  hasMore: boolean;
  loadMore: () => Promise<void>;
}

export function useEuropeAirportSearch(
  initialQuery: string = '',
  options: UseEuropeAirportSearchOptions = {}
): UseEuropeAirportSearchReturn {
  const {
    debounceMs = 300,
    cacheResults = true,
    fallbackToApi = false,
  ...
  const [results, setResults] = useState<EuropeAirportSearchResult[]>([]);
  ...
  const popularAirports = useMemo(() => {
    return europeAirportSearch.getPopularAirports(10);
  }, []);
  ...
  const debouncedSearch = useCallback(
    async (query: string) => {
      ...
      try {
        ...
        const searchResults = await europeAirportSearch.searchAirports(query, {
          ...searchOptions,
          limit: currentLimit
        });
  ...
  const getAirportByCode = useCallback((code: string) => {
    return europeAirportSearch.getAirportByCode(code);
  }, []);
